...
if (isset($_GET['logout'])) {
	...
	$query = "SELECT * FROM patients WHERE UserID = " . $_SESSION['UserID'];
	...
	if ($result) {
		...
		echo "Error fetching patient details: " . mysqli_error($mysqli);
	}
...
if ($doctorprofile) {
	...
	$querydoctor = "SELECT * FROM doctor WHERE DoctorID = " . $_SESSION['DoctorID'];
...
if (isset($_POST['treatmentHistory'])) {
	$doctorsID = $_SESSION['DoctorID'];
    $patientsID = $_SESSION['UserID'];
	...
	header('../presentaionlayer/patient/myinfo.php');
		...
		<tr class="t">
		<th class="h">DoctorID</th>  
		<th class="h">DoctorName</th>
		<th class="h">Treatment</th>
		<th class="h">Doctor's Note</th>	
		</tr> 
		
	<?php  
		$sql11="SELECT  * FROM  description,  doctor WHERE descID='$patientsID' AND doctorIDdesc=DoctorID" ;
